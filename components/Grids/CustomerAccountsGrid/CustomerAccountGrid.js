...
export default function CustomerAccountGrid(props) {
    const { customer_id, updateCustomerAcountGrid } = props
    ...
    useEffect(() => {
        const fetchData = async () => {
            const customerAccountsList_ = await customerAccounts.findAllByCustomerId(customer_id)
            setCustomerAccountsList(customerAccountsList_)
        }
        fetchData()

    }, [updateCustomerAcountGrid, customer_id])

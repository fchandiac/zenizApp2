...
export default function NewReception() {
    ...
    const saveReception = async () => {
        if (reception.packs.length == 0) {
            ...
            let money = 'CLP'
            if (receptionUsd > 0 && receptionClp == 0) { money = 'USD' }
            try {
                ...
                reception.packs.forEach(async (pack) => {
                    await pallets.updateTrays(pack.pallet.id, pack.quanty)
                })
                ...
                await records.create('Recepciones','Creación','Recepción ' + newReception_.id,user.id)
